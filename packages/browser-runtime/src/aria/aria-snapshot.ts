...
import ariaBundleAsset from "./aria-snapshot.bundle.txt" with { type: "text" };
...
function decodeAriaBundleAsset(asset: string): string {
	const modulePrefix = "export default ";
	if (asset.startsWith(modulePrefix) && asset.endsWith(";")) {
		try {
			const value: unknown = JSON.parse(asset.slice(modulePrefix.length, -1));
			if (typeof value === "string") return decodeAriaBundleAsset(value);
		} catch {}
	}
	const base64Prefix = "data:text/plain;base64,";
	if (asset.startsWith(base64Prefix)) return Buffer.from(asset.slice(base64Prefix.length), "base64").toString("utf8");
	const textPrefix = "data:text/plain,";
	if (asset.startsWith(textPrefix)) return decodeURIComponent(asset.slice(textPrefix.length));
	if (!asset.includes("module.exports")) {
		throw new TypeError(`ARIA snapshot bundle asset did not load as text: ${asset.slice(0, 200)}`);
	}
	return asset;
}

const ariaBundle = decodeAriaBundleAsset(ariaBundleAsset);
...
/**
 * Build a self-contained expression that resolves one ref from the latest
 * snapshot in the current JavaScript world. Callers can retain the returned
 * remote object and invoke only fixed, broker-owned functions on it.
 */
export function buildResolveAriaRefScript(ref: string): string {
	const normalized = parseAriaRefSelector(ref);
	if (!normalized) throw new TypeError(`Invalid ARIA ref: ${ref}`);
	return `(function(){var module={exports:{}};\n${ariaBundle}\nreturn module.exports.resolveAriaRef(${JSON.stringify(normalized)});})()`;
}
