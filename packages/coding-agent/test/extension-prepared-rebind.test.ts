...
describe("prepared extension rebinding", () => {
	it("binds a fresh session extension without evaluating the module again", async () => {
		...
		const pwdCommand = process.platform === "win32" ? process.execPath : "pwd";
		const pwdArgs = process.platform === "win32" ? ["-e", "process.stdout.write(process.cwd())"] : [];
		const [parentPwd, childPwd] = await Promise.all([
			bindings[0]!.exec(pwdCommand, pwdArgs),
			bindings[1]!.exec(pwdCommand, pwdArgs),
		]);
