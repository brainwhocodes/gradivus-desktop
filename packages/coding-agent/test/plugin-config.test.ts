...
describe("plugin config", () => {
	...
	test.skipIf(process.platform === "win32")(
		"resolves marketplace settings without restoring duplicate list entries",
		async () => {
	...
	test.skipIf(process.platform === "win32")(
		"resolves a project-scoped marketplace plugin absent from the user root",
		async () => {
	...
	test.skipIf(process.platform === "win32")(
		"prefers the active project plugin over a same-named user install",
		async () => {
	...
	test.skipIf(process.platform === "win32")(
		"falls back to an enabled user plugin when the project copy is disabled",
		async () => {
			const userPkg = path.join(pluginsDir, "node_modules", "omp-commit");
			await writeManifest(userPkg, {
				omp: {
