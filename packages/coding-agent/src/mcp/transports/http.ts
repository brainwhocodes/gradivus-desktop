...
async function readJsonBody(response: Response, timeoutMs: number, callerSignal?: AbortSignal): Promise<unknown> {
	const body = response.json() as Promise<unknown>;
	if (!isMCPTimeoutEnabled(timeoutMs)) return body;
	const timeout = Promise.withResolvers<never>();
	const timer = setTimeout(() => {
		const error = new Error("MCP response body timeout");
	...
	try {
		return await Promise.race([body, timeout.promise]);
	} finally {
		clearTimeout(timer);
...
export class HttpTransport implements MCPTransport {
	...
	async #executeRequest<T>(
		...
		try {
			...
			const result: unknown = await readJsonBody(response, timeout, options?.signal);
