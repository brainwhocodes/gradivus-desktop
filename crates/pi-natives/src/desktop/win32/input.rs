...
mod foreground {
	...
	impl ForegroundGuard {
		fn activate(id: &str) -> CoreResult<Self> {
			...
			// SAFETY: `SetForegroundWindow` only validates its own argument, and
			// `target` was resolved by `background::hwnd`.
	...
	fn move_to(x: f64, y: f64) -> CoreResult<()> {
		...
		// SAFETY: `GetSystemMetrics` has no preconditions for any index used here.
