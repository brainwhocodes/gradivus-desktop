...
Opening the drawer also initializes the terminal engine selected by the host platform: Windows renders with `wterm-dom`, and macOS, Linux, and other fallback platforms render with `ghostty-web`. Both engines fetch a WebAssembly terminal core through the packaged app protocol. The engine choice is exposed on the drawer shell as `data-terminal-renderer`, and the interactive area is a region named **Shell terminal**; it is a canvas on macOS/Linux and DOM rows on Windows.
...
Input is serialized to the PTY, output is rendered locally, and drawer resize updates terminal columns and rows. On macOS/Linux the renderer measures the grid on demand when the drawer is shown; on Windows the engine's own resize observation is authoritative. Both engines report measured grid changes through the same bounded 2–500 column/row contract and suppress duplicate dimensions.
...
Typing `exit` or otherwise ending the process changes the state to `exited` and reveals **Restart shell**. Restart closes the old PTY, disposes the terminal engine, clears the presentation replay offset, and creates a new shell using current new-terminal settings and the same selected engine. On macOS/Linux a successful WebAssembly load is cached for the rest of the app session, while a failed load clears the cache so **Restart shell** performs a real retry; on Windows each shell mount loads the core again. Explicit terminal close, session/workspace switching, session-runtime restart, renderer component destruction, and application shutdown clean up the ephemeral chat PTY. Hiding alone is not completion. Durable workspace terminals are a separate container and are not closed by Local terminal cleanup.
...
| Host platform | Windows (`win32`) selects the `wterm-dom` DOM-row engine; macOS, Linux, and other platforms select the `ghostty-web` canvas engine. | The engine is fixed per platform; there is no user-facing engine switch and no automatic fallback between engines. |
...
| WebAssembly core availability | The engine fetches its packaged WebAssembly core when the shell mounts. | A failed fetch fails the shell visibly; **Restart shell** retries the same engine. A cached successful load is reused for later shells on macOS/Linux. |
...
| environment failure | Workspace-runtime disconnection, PTY creation failure, write failure, terminal error, or an unfetchable WebAssembly core leaves the drawer visible when possible, changes status to `failed` or shows an alert, and offers restart. The drawer does not silently fall back to the other terminal engine, because that would hide a broken packaged asset. There is no offline command queue. |
...
- The drawer has no visible title; the outer section is accessibly named **Local chat terminal** and the interactive area is a region named **Shell terminal**. Tests and UI logic address the region rather than assuming a canvas child, because Windows renders DOM rows.
- `data-terminal-renderer` on the shell shows the selected engine (`wterm-dom` on Windows, `ghostty-web` elsewhere), which makes a misrouted platform visible in the running app.
...
- On Windows the engine's own resize observation is authoritative; the explicit fit pass after mount is a no-op there, while macOS/Linux fit on every show.
...
- **Open question:** Renderer reload, drawer resizing while a process emits output, workspace-runtime reconnect, and session switching have not been exercised end to end in Electron on the Windows engine.
...
- **Open question:** The macOS/Linux journey for the renderer cutover has not been rerun on a macOS host in this pass; its platform assertions are symmetric in the shared spec but the executed evidence here is Windows x64.
